...
interface IAdminUserGroupDeleteModal {
  usergroups?: IAdminUserGroup[]
  disclosure: UseDisclosureProps
  onComplete: (idsToRemove: string[]) => void
}

const AdminUserGroupDeleteModal: FC<IAdminUserGroupDeleteModal> = ({usergroups, disclosure, onComplete}) => {
  ...
  const handleSubmit = useCallback(async () => {
    setLoading(true)
    try {
      await Promise.all(usergroups.map((usergroup) => AdminUserGroups.Delete(usergroup?.ID)))
...
export default AdminUserGroupDeleteModal
