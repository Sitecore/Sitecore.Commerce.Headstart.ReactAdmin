import {QueryValidator, RuleType, RuleGroupTypeAny, ValidationMap, ValidationResult} from "react-querybuilder"
import {combinators} from "./combinators"

export const validator: QueryValidator = (query) => {
  const result: ValidationMap = {}

  validateGroup(query, result)
...
function validateGroup(group: RuleGroupTypeAny, result: ValidationMap) {
  ...
  group.rules.forEach((r) => {
    if (typeof r === "string") {
      ...
      validateGroup(r, result)
    } else {
      validateRule(r, group, result)
    }
  })
}

function validateRule(rule: RuleType, group: RuleGroupTypeAny, result: ValidationMap) {
  const reasons: any[] = []

  if (rule.field.startsWith("LineItem.") && !group["operator"]?.startsWith("items.")) {
    reasons.push({
      code: "lineItemFieldWithoutItemsGroup",
      message: "Products, Variants, and Shipping Address must use the line item group operators"
    })
  }

  if (group["operator"]?.startsWith("items.") && !rule.field.startsWith("LineItem.")) {
    reasons.push({
      code: "invalidFieldForItemsGroup",
      message: "The line item group operators can only be used with Product, Variant, and ShippingAddress fields"
    })
  }
